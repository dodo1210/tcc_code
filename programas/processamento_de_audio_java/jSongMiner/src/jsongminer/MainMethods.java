...
public class MainMethods
{
	...
	/**
	 * Prepare a MusicBrainzMiner object to mine Music Brainz. Set it to be
	 * associated with the song with the specified title and artist name. Print
	 ...
	 * @param error_stream				To print error updates to. Nothing
	 *									printed if this is null.
	 * @return							A MusicBrainzMiner associated with the
	 *									specified song. Null if no association
	 *									could be made.
	 */
	public static MusicBrainzMiner matchToMusicBrainzTrack( String model_song_title_metadata,
			String model_artist_metadata,
			PrintStream status_stream,
			PrintStream error_stream )
	{
		MusicBrainzMiner mb_miner = null;

		if (status_stream != null)
			status_stream.println("Attempting to match the song to a Music Brainz track...\n");

		try
		{
			if (model_song_title_metadata == null)
				throw new Exception("No model song title to base identification upon available.");
			mb_miner = new MusicBrainzMiner(model_song_title_metadata, model_artist_metadata);
			if (status_stream != null)
				status_stream.println("Music Brainz successfully matched song: " + model_song_title_metadata + ", by " + model_artist_metadata + "\n");
		}
		catch (Exception e)
		{
			if (error_stream != null)
				error_stream.println("Music Brainz song identification FAILED!: " + e.getMessage() + "\n");
		}
	...
	/**
	 * Extracts all available song and, if appropriate, artist and album
	 * metadata from the Music Brainz API for the song stored in it. Adds
	 * the extracted metadata to the Vector parameters of this method. Print
	 * status or error messages as appropriate.
	 *
	 * <p>Song metadata is always extracted. Artist and album metadata are only
	 ...
	 * as it is left to the calling object to do this.
	 *
	 * @param mb_miner					A MusicBrainzMiner associated with a
	 *									particular specified song. Null if no
	 *									association could be made with a
	 *									particular song, in which case no
	 *									extraction is attempted.
	 ...
	 *									problem occurs.
	 */
	public static void extractMusicBrainzMetadata( MusicBrainzMiner mb_miner,
			Vector<MusicMetaData> song_metadata,
			Vector<MusicMetaData> artist_metadata,
			Vector<MusicMetaData> album_metadata,
			HashMap< String, Vector<MusicMetaData> > artist_metadata_so_far,
			...
			throws Exception
	{
		...
		{
			if (status_stream != null)
				status_stream.println("Attempting to extract Music Brainz metadata...\n");
			boolean music_brainz_found_metadata = false;
			music_brainz_found_metadata = mb_miner.getAllAvailableNewMetaData( song_metadata,
					artist_metadata,
					album_metadata,
					artist_metadata_so_far,
					album_metadata_so_far,
					store_fails );
			if (music_brainz_found_metadata)
			{
				if (status_stream != null)
					status_stream.println("Music Brainz metadata successfully extracted.\n");
			}
			else if (error_stream != null)
				error_stream.println("Music Brainz metadata extraction FAILED!\n");
		}
	}
	...
	/**
	 * Parses a HashMap from the specified
