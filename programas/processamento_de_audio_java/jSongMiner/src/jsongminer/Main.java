	...
	public static void main(String[] args)
	{
		try
		{
			...
			if ( configs.enable_echo_nest_fingerprinting ||
				 ...
				 configs.save_music_brainz_metadata )
			{
			...
			if (identifying_title != null)
			{
				...
				else
				{
					...
					// Attempt to match to a Music Brainz song and extract Music
					// Brainz metadata (song and, if appropriate, artist and
					// album) for the newly matched song
					if (configs.save_music_brainz_metadata)
					{
						MusicBrainzMiner mb_miner = MainMethods.matchToMusicBrainzTrack( identifying_title,
							identifying_artist,
							status_stream,
							error_stream );
						MainMethods.extractMusicBrainzMetadata( mb_miner,
							song_metadata,
							artist_metadata,
							album_metadata,
							artist_metadata_so_far,
