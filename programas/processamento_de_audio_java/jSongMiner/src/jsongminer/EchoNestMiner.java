...
public class EchoNestMiner
{
			...
			throws Exception
	{
		...
		// Extract the Echo Nest release name
		try {results.add(createAndStoreMusicMetaDataObject("Echo Nest Release Name", song.getReleaseName(), store_result));}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Echo Nest Release Name", "Could not extract this information: " + e.getMessage(), store_result));}

		// Extract the 7-Digital-US song ID
		try
		{
			Track track = song.getTrack("7digital-US");
			String full_id = track.getForeignID();
			String stripped_id = cleanForeignID(full_id, "7digital-US:track:");
			results.add(createAndStoreMusicMetaDataObject("7digital-US Song ID", stripped_id, store_result));
		}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("7digital-US Song ID", "Could not extract this information: " + e.getMessage(), store_result));}
		...
		// Extract the Echo Nest song danceability
		try {results.add(createAndStoreMusicMetaDataObject("Echo Nest Danceability (0 to 1)", (new Double(song.getDanceability())).toString(), store_result));}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Echo Nest Danceability (0 to 1)", "Could not extract this information: " + e.getMessage(), store_result));}

		// Extract the Echo Nest song energy
		try {results.add(createAndStoreMusicMetaDataObject("Echo Nest Energy (0 to 1)", (new Double(song.getEnergy())).toString(), store_result));}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Echo Nest Energy (0 to 1)", "Could not extract this information: " + e.getMessage(), store_result));}

		// Extract the Echo Nest analysis URL
		try {results.add(createAndStoreMusicMetaDataObject("Echo Nest Analysis URL", song.getAnalysisURL(), store_result));}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Echo Nest Analysis URL", "Could not extract this information: " + e.getMessage(), store_result));}
			...
			throws Exception
	{
		...
		// Extract Rdio-US Artist ID
		try
		{
			String full_id = artist.getForeignID("rdio-us-streaming");
			String stripped_id = cleanForeignID(full_id, "rdio-us-streaming:artist:");
			results.add(createAndStoreMusicMetaDataObject("Rdio-US Artist ID", stripped_id, store_result));
		}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Rdio-US Artist ID", "Could not extract this information: " + e.getMessage(), store_result));}

		// Extract Facebook Artist ID
		try
		{
			String full_id = artist.getForeignID("facebook");
			String stripped_id = cleanForeignID(full_id, "facebook:artist:");
			results.add(createAndStoreMusicMetaDataObject("Facebook Artist ID", stripped_id, store_result));
		}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Facebook Artist ID", "Could not extract this information: " + e.getMessage(), store_result));}

		// Extract the artist's years active
		try
		{
			Long[] years_range = artist.getYearsActive().getRange();
			if (years_range[1] == null) years_range[1] = new Long(java.util.Calendar.getInstance().get(java.util.Calendar.YEAR));
			String years_string = years_range[0] + " to " + years_range[1];
			results.add(createAndStoreMusicMetaDataObject("Years Active", years_string, store_result));
		}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Years Active", "Could not extract this information: " + e.getMessage(), store_result));}
		...
		// Extract the Echo Nest artist-related songs
		try
		{
			List<Song> song_list = artist.getSongs();
			int this_max = song_list.size();
			if (max_to_report < this_max) this_max = max_to_report;
			for (int i = 0; i < this_max; i++)
			{
				Song this_song = song_list.get(i);
				results.add(createAndStoreMusicMetaDataObject("Artist-Related Song (Song " + (i+1) + ")", this_song.getTitle(), store_result));
			}
		}
		catch (Exception e) {if (store_fails) results.add(createAndStoreMusicMetaDataObject("Artist-Related Song", "Could not extract this information: " + e.getMessage(), store_result));}
