...
/**
 * A feature calculator that finds the bin index of the higher (in terms of bin index) of the two
 * tempo-standardized beat histogram peaks with the highest magnitude, divided by the index of the lower (in
 * terms of bin index) of the two bins.
 *
 * @author Cory McKay
 */
public class HarmonicityOfTwoStrongestRhythmicPulsesTempoStandardizedFeature
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			{
				if ( sequence_info.beat_histogram_thresholded_table_120_bpm_standardized[bin][1] > second_highest_bin_magnitude && 
				     bin != max_index )
				{
					second_highest_bin_magnitude = sequence_info.beat_histogram_thresholded_table_120_bpm_standardized[bin][1];
